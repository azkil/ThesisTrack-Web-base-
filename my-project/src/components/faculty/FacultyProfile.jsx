...
export default function FacultyProfileEdit() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await axios.get(`http://localhost:3001/api/faculty/${facultyId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!payload.password) {
        const currentRes = await axios.get(`http://localhost:3001/api/faculty/${facultyId}`);
        payload.password = currentRes.data.password;
      }
      const res = await axios.put(`http://localhost:3001/api/faculty/${facultyId}`, payload);
      setProfile({ ...res.data, password: "" });
  ...
  return (
    <div className="space-y-4 p-3 sm:p-4 lg:p-6">
      <PageHeader title="Faculty Profile" description="Update your faculty account details." />
